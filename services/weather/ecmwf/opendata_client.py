"""Optional ecmwf-opendata retrieval for deterministic IFS forecasts."""
...
def _client_factory(**kwargs: Any) -> Any:
    try:
        ...
        raise RuntimeError(
            "ECMWF retrieval requires the optional weather ingestion extras"
        ) from error
    return Client(**kwargs)


class EcmwfIfsOpenDataClient:  # pylint: disable=too-few-public-methods
    """Retrieve explicit IFS fields with a small, bounded retry policy."""
    ...
    def __init__(
        ...
        *,
        ...
        if maximum_retries < 0 or retry_after < 0:
            raise ValueError("ECMWF retry bounds must not be negative")
    ...
    def retrieve(self, request: RetrievalRequest) -> RetrievedArtifact:
        """Retrieve one immutable IFS request into its destination directory."""
        request.target_dir.mkdir(parents=True, exist_ok=True)
        target = request.target_dir / _filename("ifs", request)
        ...
        return RetrievedArtifact.from_path(target, "ecmwf-ifs", request)
