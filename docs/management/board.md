...
| EV-SAT-001 | Satellite (Himawari-8/9, Sentinel-1/2, Landsat 8/9) | **connected + persisted + API served (2026-08-25)**: Himawari band3 segment retained and queryable; field decode pending JMA guide; Everest AWS display/research only | docs/meteorology/weather-spec.md |
| EV-TERRAIN-001 | Terrain/GIS (Copernicus DEM GLO-30, Cesium/3D Tiles, PostGIS) | **connected + persisted + API served (2026-08-25)**: GLO-30 tile in persistent DB, `GET /api/terrain/tile` returns it; Cesium 3D Tiles/PostGIS rendering future | docs/gis/terrain-spec.md |
