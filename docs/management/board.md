...
| EV-PHASE2-GFS | Activate NOAA GFS scheduled ingestion | backend + meteorology + operations | **CANARY + FIRST AUTOMATIC CYCLE PASS (2026-08-28):** dedicated `/opt/everest/worker-venv` carries xarray 2026.7.0, cfgrib 0.9.15.1, netCDF4 1.7.4, Herbie 2026.3.0, ecmwf-opendata 0.3.34, and ecCodes 2.48.0. Fixed Herbie 2026.x source verification to use matching `grib_source`/`idx_source`. Official AWS NOAA GFS 2026-08-27 12Z canary ingested F000/F003/F006; the first automatic IFS+GFS systemd cycle then completed with exit 0, 206 records total, IFS and GFS both `succeeded`, AIFS/ICON skipped. GFS persisted 25 leads F000..F072 with visibility and 25 regional 400 hPa 5x4 frames; registry is `connected/healthy` and run history is `succeeded`. Runtime was 21m21s with 5.8G peak memory and raw storage reached 1.1G, so the worker now disables cfgrib geography caching and carries `MemoryHigh=6G`, `MemoryMax=8G`, `RuntimeMaxSec=1h`, and `Nice=10` guards. | Observe one more scheduled run, raw growth, and memory peak before enabling AIFS | `infra/docker/systemd/everest-scheduler.service`, `apps/api/everest_api/scheduler/provider_jobs.py`, `apps/api/schedule_forecast.py`, `services/weather/retrieval.py` |
| EV-ENG-HARDEN-001 | API/Worker/Web containers + CI | backend + frontend + operations | **CONTAINERS IMPLEMENTED / CI WORKFLOW BLOCKED BY GITHUB SCOPE (2026-08-28):** added non-root Python 3.13 API and ingestion-worker images, non-root Node 22 web image with reproducible Cesium asset generation, one-shot Alembic migration, and Compose `app` and `jobs` profiles while default remains PostgreSQL-only. A GitHub Actions workflow for PostgreSQL-backed Python tests, frontend tests/lint/typecheck/build, Gitleaks, and no-push Docker builds is locally defined and validated, but both HTTPS OAuth and GitHub App credentials lack workflow-write scope, so GitHub rejected publication and the workflow is not claimed as remote. Local Compose/YAML/config, 357 PostgreSQL-enabled Python tests (5 retained-real-data skips), and 152 frontend tests passed. CI-equivalent Python tests separate weather-native, API, and PostgreSQL-native pytest processes because combining ecCodes/netCDF and psycopg binary stacks in one Python 3.13 process triggers an allocator abort only during interpreter teardown. Local Dockerfile parsing reached base-image resolution, but actual image builds were blocked by repeated Docker Hub TLS/network timeouts before any Dockerfile build step. | Authorize `workflow` scope, publish the validated workflow, then require all four jobs; Next/PostCSS/Sharp high advisories remain a separate Next 16 migration | `infra/docker/*.Dockerfile`, `infra/docker/compose.yml`, `infra/docker/README.md`, local branch `ci-workflow-local` |
