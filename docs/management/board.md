...
| EV-GATEC-OP-RETENTION-002 | WORM, legal hold, and disposition | B2 offline remediation + role reconciliation COMPLETE (2026-08-25); Stage 1-3 nonproduction evidence QA PASS; Governance QA activation and Stage 4 canary pending separate gates | AWS role state reconciled with remediated contract (mis-created roles deleted; broker/admin/audit uncreated; legal/hold/disposition absent); bucket + policy retained; legal hold/disposition prohibited; canary disabled | `docs/management/decisions.md`, `docs/architecture/EV-GATEC-OP-RETENTION-002-backend-contract.md` |
...
| EV-GATEC-OP-RELEASE-010 | Release gate definition | DEFINED (2026-08-25); no deployment authorized; requires Manager release decision + CloudTrail delivery checkpoint | Gate C-Operational QA pass (QA-009) | `docs/architecture/EV-GATEC-OP-RELEASE-010.md` |
...
| --- | --- | --- | --- | --- |
| EV-UI-001 | Frontend UI (deferred A-F display) | frontend + gis-3d | **Integration QA FAIL / NO-GO (2026-08-25):** 21 web tests, ESLint, configured build/typecheck pass, but CORS preflight returns 405, API datetimes violate UTC `Z`, forecast time filters current records, only one old ICON weather record is available, and no usable frontend runtime was evidenced. Historical A-F source/core acceptance is unchanged; current four-source integration and correct frontend display are not established. | `docs/product/PRD.md`, `docs/architecture/architecture.md`, `docs/design/DESIGN.md`, `docs/qa/test-plan.md` |
| EV-AWS-STATION-001 | Everest AWS observation stations | meteorology + backend | **connected + canonical + API + integration PASS; review High/Med/Low all resolved** (ADR-019 amendment; atomic download; parser bounds guard; black 80-col clean); pending QA | docs/meteorology/weather-spec.md |
| EV-PYRAMID-001 | Pyramid meteorological network | meteorology + backend | planned (registration required) | docs/meteorology/weather-spec.md |
| EV-SAT-001 | Satellite (Himawari-8/9, Sentinel-1/2, Landsat 8/9) | meteorology + gis-3d | **connected + canonical + API + integration PASS; review High/Med/Low all resolved** (real sha256; no leak; SQL dedup; mocked network test; atomic download); field decode pending JMA guide; pending QA | docs/meteorology/weather-spec.md |
| EV-TERRAIN-001 | Terrain/GIS (Copernicus DEM GLO-30, Cesium/3D Tiles, PostGIS) | gis-3d | **connected + canonical + API + integration PASS; review High/Med/Low all resolved** (no leak; input validation; atomic download; black clean); pending QA | docs/gis/terrain-spec.md |
| EV-ENV-OSM-AI-RISK | Environmental / OSM / AI / Risk | tbd | blocked on individual scope | - |
