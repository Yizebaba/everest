...
| EV-PHASE2-GFS | Activate NOAA GFS scheduled ingestion | backend + meteorology + operations | **CANARY PASS + SCHEDULER ENABLED (2026-08-28):** installed a dedicated `/opt/everest/worker-venv` with xarray 2026.7.0, cfgrib 0.9.15.1, netCDF4 1.7.4, Herbie 2026.3.0, ecmwf-opendata 0.3.34, and ecCodes 2.48.0. Fixed Herbie 2026.x source verification to use matching `grib_source`/`idx_source`. Official AWS NOAA GFS cycle 2026-08-27 12Z canary ingested F000/F003/F006: 3 raw/canonical records, run outcome `succeeded`, registry `connected/healthy`; temperature/wind/visibility are served by `/api/weather/current?source=noaa-gfs`. GFS regional 400 hPa frames materialized at 12Z/15Z/18Z, each 5x4 native grid (20 points). systemd scheduler now runs from the worker venv with persistent `EVEREST_RAW_ROOT`, enables IFS+GFS only, and leaves AIFS/ICON disabled. | Observe the next two scheduled runs and degrade/rollback if GFS is not continuously healthy | `infra/docker/systemd/everest-scheduler.service`, `apps/api/everest_api/scheduler/provider_jobs.py`, `apps/api/schedule_forecast.py` |
