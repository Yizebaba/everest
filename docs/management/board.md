...
| EV-PHASE1-GLUE | Open-source weather/domain glue Phase 1 | backend + meteorology + frontend + gis-3d | **IMPLEMENTED / REVIEW FIXES COMPLETE (2026-08-28):** IFS regional materialization uses `cfgrib`/`xarray` over retained pressure-level GRIB and exact AOI `27.5..28.5 N`, `86.4..87.4 E`; direct ecCodes remains canonical point ingestion. The API accepts exact IFS/GFS source-model pairs and implements exact UTC `valid_time` selection with no latest fallback. GFS remains connector/configured only; no real served GFS regional frame is claimed. Frontend uses exact backend grid-node seeds, skips missing nodes, hard-caps 256 emitters, applies one factual U/V velocity source, and transactionally rolls back partial setup. The 400 hPa field is displayed on a fixed 7,500 m non-geometric plane using Cesium's public `ParticleSystem`; no private renderer API or handwritten shader is used. `/api/risk/summit-window` and typed domain views remain separate completed surfaces. Backend 314 passed/37 skipped; frontend 140 passed plus lint/typecheck/build. | Optional ingestion extras and an enabled scheduler run before claiming real GFS regional output; pre-existing npm audit highs remain tracked | `docs/design/regional-wind-field.md`, `docs/api/API.md`, `docs/meteorology/weather-spec.md` |
