...
| EV-GATEC-OP-RETENTION-002 | WORM, legal hold, and disposition | Stage 1 offline foundation COMPLETE + QA recorded (24 tests PASS); Stage 2 Governance provisioning is next | Governance provisioning requires separate task authorization + MFA admin session (human); legal hold/disposition prohibited; canary disabled until QA PASS | `docs/management/decisions.md`, `docs/architecture/EV-GATEC-OP-RETENTION-002-backend-contract.md` |
...
| --- | --- | --- | --- | --- |
| EV-UI-001 | Frontend UI (deferred A-F display) | frontend + gis-3d | PRD→design-review COMPLETE; **implementation: scaffold + 8-API client + lib + i18n + 7 panels (SummitWindow/Forecast/Ladder/Disagreement/Sources/Provenance/TimeNav) + Cesium scene with real record markers + terrain/satellite layer toggles; ports 48237/50149; typecheck ✓, 20 tests ✓, prod build ✓, eslint ✓, prettier ✓**; remaining: scene record→provenance polish, more tests, /review | `docs/product/PRD.md`, `docs/architecture/architecture.md`, `docs/design/DESIGN.md` |
